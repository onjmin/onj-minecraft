...
export const bedrockSkills = [
	...
	// 世界が置いた物資を取るための唯一の手段。
	// 2026-09-21 06:55 に一度外した。06:32 に他プレイヤーの拠点(松明に囲まれた
	// (131..133, 63..64, -113..-111) の 4 つのチェスト)を空にし、世界が置いた
	// チェストと人のチェストを LLM が見分けられていなかったため。
	// 同日、オーナーの判断で戻した。持ち出したのはゴミアイテムで返却不要、
	// 盗んだベッドで復帰地点を作るのも可。Realm で出た荒らしの苦情は初期リスの
	// 穴掘りと会話への乱入が原因で、略奪とは別系統。レア品(ダイヤ装備・ネザー
	// 装備)を取った場合だけ返す。この見分けは LLM の判断に任せ、コードで分岐は
	// 足さない(judgment-belongs-to-llm-not-code)。
	stealFromChestSkill,
