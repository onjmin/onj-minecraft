 ...
 */
export const stealFromChestTool = createTool<void, { itemsCount: number; containerType: string }>({
	name: "collecting.stealing",
	description: "Finds a nearby chest or barrel, opens it, and takes all items inside.",
	inputSchema: {} as any,
	handler: async (
		agent: AgentOrchestrator,
	): Promise<ToolResponse<{ itemsCount: number; containerType: string }>> => {
		const { bot } = agent;
		...
		if (!containerBlock) {
			return toolResult.fail("No chests or barrels found nearby.");
		}
		...
		try {
			...
			if (itemsCount === 0) {
				container.close();
				return toolResult.ok("The container was empty.", {
					itemsCount: 0,
			...
			return toolResult.ok(`Stole ${itemsCount} items from ${containerBlock.name}.`, {
				itemsCount,
				ate,
				equipped,
				containerType: containerBlock.name,
