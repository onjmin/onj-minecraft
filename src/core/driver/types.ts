...
export interface BotDriver {
	...
	/**
	 * 直前の操作が「持ち物の識別子のずれ」で拒否されたか。
	 *
	 * 統合版は拾った物の識別子(StackNetworkID)を送ってこないので、こちらの
	 * 予測とサーバーの中身がずれる。ずれた山は持ち替えもクラフトも
	 * status=49 で拒まれ、何度やっても同じ理由で弾かれる。直し方は
	 * 「実際に持ち物を変えて一覧を送り直させる」しかなく、それはブロックを
	 * 置ける層(スキル)の仕事なので、ここでは事実だけを返す。
	 * Java版は常に false。
	 */
