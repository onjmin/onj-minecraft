### onj-minecraftの概要
- 全体的にProject Sidのパクリ。
- MindCraftが抱える「行動成功率の低さ」を、泥臭いフォールバック処理で無理やり解決する。
- 行動のサイクルからLLMを追い出し、RTX 5060 Ti 程度のアフガキ環境でも複数体を快適に回すことを目指す。

### きっかけ
MindCraftを試運転したところ「ダイヤを斧で叩くアホ」だと判明してしまったため。
...
* 人間の指示の遂行に特化した国内事例。Function Callingの活用により、実行速度を高速化している。

### onj-minecraftの要件
- 各エージェントの人間性はプロフィール定義で管理。
- 各エージェントは反復行動を行う。
- 非同期でLLMがプロフィールを読み込み、そのエージェントの次の反復行動を決定する。
